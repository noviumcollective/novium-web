...
export interface ButtonProps
    extends React.ButtonHTMLAttributes<HTMLButtonElement>,
        VariantProps<typeof noviumButtonVariants> {
    asChild?: boolean;
}

const NoviumButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant, size, asChild = false, ...props }, ref) => {
        const Comp = asChild ? Slot : "button";
        return (
            <Comp
                className={cn(
                    noviumButtonVariants({ variant, size, className })
                )}
                ref={ref}
                {...props}
            />
...
NoviumButton.displayName = "NoviumButton";

export { NoviumButton, noviumButtonVariants };
